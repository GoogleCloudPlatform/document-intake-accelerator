...
def test_reassign_user_journey(setup):
  ...
  files1 = [("files", ("California14.pdf", open(TESTDATA_APPLICATION1,
                                                "rb"), "application/pdf")),
            ("files", ("california-dl-18.pdf",
                       open(TESTDATA_SUPPORTING_DOC_DL,
                            "rb"), "application/pdf")),
            ("files", ("california-paystub-14.pdf",
                       open(TESTDATA_SUPPORTING_DOC_PAYSTUB,
                            "rb"), "application/pdf")),
            ("files", ("california-utility-bills-14.pdf",
                       open(TESTDATA_SUPPORTING_UTILITY_BILL,
                            "rb"), "application/pdf"))]

  files2 = [("files", ("California18.pdf", open(TESTDATA_APPLICATION2,
                                                "rb"), "application/pdf"))]
  ...
  response = requests.post(
      upload_base_url + f"/upload_service/v1/process_task", json=payload_app2)
  assert response.status_code == 202
  time.sleep(60)
  response = requests.post(
      upload_base_url + f"/upload_service/v1/process_task", json=payload_app1)
  assert response.status_code == 202
