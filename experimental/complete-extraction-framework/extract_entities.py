...
#                                     gcs_doc_path.split("/")[-1][:-4])
  # FOLDER ADDED TO SAVE JSON AND USE IT FOR TABLE EXTRACTION
...
if __name__ == "__main__":
   ...
   os.environ[
     'GOOGLE_APPLICATION_CREDENTIALS'] = \
         "/Users/sumitvaise/Downloads/claims-processing-dev-58abd8292d88.json"
   ...
   state = "arizona"
   gcs_doc_path = \
       "gs://claim_form_arizona/new_files/Copy of Arizona_claim182.pdf"
   json_out = extract_entities(gcs_doc_path, doc_type, state)[0]
   import pandas as pd
   import json
   if isinstance(json_out, str):
    df = pd.DataFrame(json.loads(json_out))
   elif isinstance(json_out, list):
    df = pd.DataFrame(json_out)
   df.to_csv('claims_182.csv', index=False)