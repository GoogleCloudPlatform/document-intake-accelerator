...
def get_extraction_score(case_id: str, uid: str, document_class: str,document_type:str,context : str):
  ...
  req_url = f"{base_url}?case_id={case_id}&uid={uid}" \
    f"&doc_class={document_class}&document_type={document_type}&context={context}"
  response = requests.post(req_url)
  return response
...
def extract_documents(doc:Dict,document_type):
  ...
  extract_res = get_extraction_score(case_id, uid, document_class,document_type,context)
