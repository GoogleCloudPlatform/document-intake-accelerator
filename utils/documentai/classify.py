...
from utils.classification.split_and_classify import get_classification_predictions
...
def process_dir(dir_uri):
  ...
  urls = []
  for blob in blobs:
    urls.append(f"gs://{bucket_name}/{blob.name}")
...
def process_file(file_uri):
  ...
  if not stats:
    ...
    for uri in classification_output:
      print(uri)
      uri_output = classification_output[uri]
      for out in uri_output:
        print(f"  {out}")
