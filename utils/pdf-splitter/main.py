...
def get_processor(
    ...
    processor_name = client.processor_path(project_id, multi_region_location, processor_id)
...
def process_file_online(client, project_id, multi_region_location, file_path, processor_id, output_dir):
    processor_name = get_processor(client, project_id, multi_region_location, file_path, output_dir, processor_id)
...
def process_file_batch(client, project_id, multi_region_location, dir_path, processor_id, output_dir):
    ...
    if args.file_uri:
        if args.file_uri.startswith("gs://"):
            ...
            if not os.path.isfile(file_path):
                ...
                return 1
            process_file_online(client, args.project_id, args.multi_region_location, file_path, args.processor_id, args.output_dir)

    if args.dir_uri:
        if args.dir_uri.startswith("gs://"):
            process_file_batch(client, args.project_id, args.multi_region_location, args.dir_uri, args.processor_id, args.output_dir)
        else:
...
def upload_to_gcs(local_path, bucket_name, prefix):
    ...
    for local_file in glob.glob(local_path + '/**'):
        if not os.path.isfile(local_file):
            upload_to_gcs(local_file, bucket, prefix + "/" + os.path.basename(local_file))
        else:
...
def split_pdf(entities: Sequence[Document.Entity], file_path: str, output_dir: str):
    ...
    with Pdf.open(file_path) as original_pdf:
        ...
        for index, entity in enumerate(entities):
            ...
            print(f"Creating subdocument {index + 1}: {output_filename} (confidence: {confidence})")
...
if __name__ == "__main__":
    ...
    parser.add_argument('-d', dest="dir_uri", help="Path to gs directory (gcs uri or local)")
    parser.add_argument('-f', dest="file_uri", help="Path to file (gcs uri or local)")
