...
echo "{\"lic\": \"$LOOKER_LICENSE_KEY\", \"email\": \"$LOOKER_TECHNICAL_CONTACT_EMAIL\", \"latest\":\"specific\", \"specific\":\"$PARSED_VERSION\"}">request.json
...
curl -i -X POST -H 'Content-Type:application/json' -d @request.json https://apidownload.looker.com/download -o response.txt
